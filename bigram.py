...
with open("fine_tuning_input.txt","r",encoding="utf-8") as f:
    fine_tuning_text = f.read()

chars = sorted(list(set(text).union(set(fine_tuning_text))))
...
ft_data = torch.tensor(encode(fine_tuning_text),dtype=torch.long)
ft_n = int(0.9*len(ft_data))
ft_train_data = ft_data[:ft_n]
ft_val_data = ft_data[ft_n:]
...
def get_batch_fine_tuning(split):
...
def estimate_loss_ft():
    ...
    for split in ['train','val']:
        losses = torch.zeros(eval_iters)
        for k in range(eval_iters):
            X,Y = get_batch_fine_tuning(split)
            logits,loss = model(X,Y)
            losses[k] = loss.item()
        out[split] = losses.mean()
    model.train()
